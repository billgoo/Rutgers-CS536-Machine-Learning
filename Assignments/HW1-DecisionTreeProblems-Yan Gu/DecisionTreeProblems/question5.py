...
def test_score(k, iteration, classifier):
    filename = 'data/data_k_' + str(k) + '_iter_' + str(iteration) + '.csv'
...
def show_Picture(x_data, y_data, x_label, y_label, title):
    ...
    filename = 'images/question5/Figure' + title[4] + '.png'
...
if __name__ == "__main__":
    ...
    for i in range(1, 52):
        ...
        fname = filename = 'data/data_k_' + str(k) + '_m_' + str(m) + '.csv'
    ...
    with open('data/question5.csv', 'w', newline='') as csvfile:
        spamwriter = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    datamap = pd.read_csv('data/question5.csv', names=['m','err_train','err_test','|err_train - err_test|'])
    col_m = datamap['m']
